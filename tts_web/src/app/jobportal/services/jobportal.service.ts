...
export class JobportalService {
  ...
  getAllJobPortalsByCompanyId(companyId: number): Observable<Jobportal[]> {
    return this.http.get<Jobportal[]>(
      `${this.jobPortalURL}jobportals/${companyId}`
    );
  }

  addJobPortal(jobPortal: Jobportal): Observable<Jobportal> {
    return this.http.post<Jobportal>(
      `${this.jobPortalURL}jobportal`,
      jobPortal
    );
  }
  ...
  updateJobPortal(
    portalId: number,
    jobPortal: Jobportal
  ): Observable<Jobportal> {
    return this.http.put<Jobportal>(
      `${this.jobPortalURL}jobportal/${portalId}`,
      jobPortal
    );
  }
